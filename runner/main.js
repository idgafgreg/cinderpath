...
function homeStats(){
  ...
  $('sound').textContent=profile.settings.sound?'♪':'♩';$('sound').setAttribute('aria-label',profile.settings.sound?'Mute sound':'Enable sound');$('sound').setAttribute('aria-pressed',String(profile.settings.sound));
}
function goHome(){
...
function openModal(kind){
  ...
  if(kind==='pause'){
    title='The road can wait.';kicker='TAKE A BREATH';
    ...
    title='Make the road your own.';kicker=`WARDROBE · ${fmt(profile.coins)} CINDERS`;
    ...
    if(wardrobeTab==='cloaks'){
      body+='<p class="dialog-intro">A new look for the same brave little soul. All cloaks are cosmetic, earned with cinders from your runs.</p><div class="cloak-grid">';
      body+=CLOAKS.map(c=>{const owned=profile.owned.includes(c.id),selected=profile.selected===c.id,locked=levelInfo(profile.xp).level<c.level,afford=profile.coins>=c.price;return `<div class="cloak-card ${selected?'selected':''}"><span class="cloak-art" style="--cloak:${c.color};--trim:${c.trim}"></span><strong>${c.name}</strong><p>${c.desc}</p><button data-cloak="${c.id}" ${!owned&&(locked||!afford)?'disabled':''} ${selected?'aria-pressed="true"':''}>${selected?'Equipped':owned?'Equip':locked?'Level '+c.level+' · ◆ '+c.price:'Unlock · ◆ '+c.price}</button></div>`;}).join('')+'</div>';
    }else {
    ...
    body=setting('sound','Sound & music','Soft synths, cinder chimes, and a little atmosphere.')+setting('motion','Atmospheric motion','Particles, menu drift, and impact flashes.')+setting('controls','On-screen controls','Tap buttons as an alternative to swiping.');
    ...
    title='Carry the last light.';kicker='HOW TO PLAY';
    ...
    body+=dialogButton('start','I’m ready. Let’s run. ↗');
  }
...
function frame(now){
  ...
  if(run.phase==='running'){
    ...
    view.events(events,run);sound.events(events,run.combo);sound.tick(true);
...
window.addEventListener('keydown',e=>{
  ...
  const action=keyActions[e.code];if(action){e.preventDefault();if(!e.repeat)command(run,action);}
});
let pointer=null;
...
$('world').addEventListener('pointermove',e=>{
  ...
  command(run,Math.abs(dx)>Math.abs(dy)?dx>0?'right':'left':dy>0?'slide':'jump');pointer=null;
});
for(const event of ['pointerup','pointercancel','lostpointercapture'])$('world').addEventListener(event,()=>pointer=null);
for(const button of document.querySelectorAll('[data-action]')){
  button.addEventListener('pointerdown',e=>{e.preventDefault();command(run,button.dataset.action);sound.unlock();});
  button.addEventListener('click',e=>{if(e.detail===0){command(run,button.dataset.action);sound.unlock();}});
}
